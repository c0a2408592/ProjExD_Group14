...
def draw_title():
    screen.fill((0, 0, 0))
    title = FONT_BIG.render("Mini Street Fighter", True, (255, 255, 255))
    guide = FONT_MED.render("Press ENTER", True, (200, 200, 200))
    screen.blit(title, (WIDTH // 2 - title.get_width() // 2, 200))
    screen.blit(guide, (WIDTH // 2 - guide.get_width() // 2, 320))


def draw_select(selected):
    screen.fill((20, 20, 20))
    title = FONT_MED.render("Select Battle", True, (255, 255, 255))
    screen.blit(title, (WIDTH // 2 - title.get_width() // 2, 60))

    options = ["Battle 1", "Battle 2", "Battle 3", "Quit Game"]
    ...
    guide = FONT_SMALL.render("↑↓ Select  ENTER Start", True, (200, 200, 200))
    ...
    while running:
        ...
        for event in pg.event.get():
            ...
            # ===== バトル選択 =====
                ...
                if event.type == pg.KEYDOWN:
                    if event.key == pg.K_UP:
                        selected_battle = (selected_battle - 1) % 4
                    if event.key == pg.K_DOWN:
                        selected_battle = (selected_battle + 1) % 4
                    if event.key == pg.K_RETURN:
                        if selected_battle == 3:   # Quit Game
                            running = False
                        else:
                            game_state = BATTLE
                            hud.reset_timer()
